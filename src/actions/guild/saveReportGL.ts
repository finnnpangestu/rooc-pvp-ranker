...
import type { PartySetup, MemberMatchReport, ActionResult } from '@/types'
  ...
  try {
    ...
    const memberReportsToSave: MemberMatchReport[] = (reportData.member_reports || []).map(
      (mr) => ({
        ...
        actual_score: mr.actual_score ? Number(mr.actual_score) : 0,
    ...
    if (charIds.length > 0) {
      ...
      for (const memberReport of reportData.member_reports || []) {
        ...
        const totalScore =
          (Number(char.gl_total_score) || 0) + (Number(memberReport.actual_score) || 0)
        const presentCount =
          (Number(char.gl_present_count) || 0) + (memberReport.is_present ? 1 : 0)
    ...
    // 4. Update party setup: save formation updates & swaps
